...
export class HelperDateService implements IHelperDateService {
  ...
  extractDate(date: string | Date | number): IHelperDateExtractDate {
    const newDate: Date = this.create(date);
    ...
    switch (dateRangeType) {
      ...
      case 'MONTH':
        if (month !== undefined && year !== undefined) {
          startTime = moment
            .utc([year, month - 1]) //-1 because moment take month from 0-11
            .add(offsetMin, 'minutes')
            .startOf('month');
          endTime = moment
            .utc([year, month - 1])
  ...
  getTodayStartDateTime(): string {
    const todayStartDateTime = new Date()
      .toISOString()
      .replace('T', ' ')
      .substring(0, 10)
      .concat(' 00:00:00.000+00');
    return todayStartDateTime;
  }

  getTodayEndDateTime(): string {
    const todayEndDateTime = new Date()
      .toISOString()
      .replace('T', ' ')
      .substring(0, 10)
      .concat(' 23:59:59.000+00');
    return todayEndDateTime;
  }

  getDaysLeft(eventStartDate: Date, registrationDate: Date) {
    const startDate = new Date(eventStartDate);
    const regDate = new Date(registrationDate);
    const diffInMs = startDate.getTime() - regDate.getTime();
    const diffInDays = Math.floor(diffInMs / (1000 * 60 * 60 * 24));
    return diffInDays;
  }

  getHoursLeft(eventStartDate: Date, registrationDate: Date) {
    const startDate = new Date(eventStartDate);
    const regDate = new Date(registrationDate);
    const diffInMs = startDate.getTime() - regDate.getTime();
